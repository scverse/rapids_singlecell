...
* Add ``GuideAssignment`` to ``ptg`` for GPU-accelerated guide RNA assignment with batched Poisson–Gaussian EM, replacing pertpy's sequential per-guide MCMC {smaller}`S Dicks`
