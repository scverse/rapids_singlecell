...
* Add 64-bit ``indptr``/``indices`` support throughout the sparse pipeline. Nanobind CUDA kernels are now templated on the index dtype with bindings registered for both ``int32`` and ``int64``, enabling datasets with ``nnz > 2³¹-1`` once CuPy ships int64 sparse support {pr}`620` {smaller}`S Dicks`
...
* Fix integer overflow in `co_occurrence` for very larg datasets {pr}`671` {smaller}`S Dicks`
