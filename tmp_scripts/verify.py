...
def build_rect_grid(n=3):
    n_cells = n * n
    coords = np.stack(np.meshgrid(np.arange(n), np.arange(n), indexing="xy"), -1).reshape(-1, 2).astype(np.float32)
    rows, cols, data = [], [], []
...
def verify_invariants_and_remaps(max_neighs=4):
    ...
    # Invariants
    assert cp.all(old_to_new[sat] == cp.arange(n_sat)), "sat positions not contiguous at front"
    assert cp.all(old_to_new[unsat] == cp.arange(n_sat, n_cells)), "unsat positions not after sat"
    ...
    assert cp.all(nearest_sat_mapped >= 0) and cp.all(nearest_sat_mapped < n_sat), "nearest_sat must map into [0, n_sat)"
    ...
    for k in range(n_sat):
        ...
        cp.testing.assert_allclose(s1, s2, rtol=0, atol=0, err_msg=f"neighbor sum mismatch at sat row {k}")
...
def verify_single_step_update(max_neighs=4, dt=1e-2):
    ...
    neighbor_sum_reo = cp.array([conc_reo[sat_idx_mapped[i]].sum() for i in range(n_sat)])
    ...
    for i in range(len(unsat)):
        ...
        conc_orig2[u_global] = cp.maximum(0.0, conc_orig2[u_global] + d2_sat_orig[ns_pos_orig] * dt)
        ...
        u_reo = n_sat + i
        conc_reo2[u_reo] = cp.maximum(0.0, conc_reo2[u_reo] + d2_sat_reo[ns_pos_reo] * dt)
    ...
    cp.testing.assert_allclose(conc_orig2, conc_reconstructed, rtol=0, atol=1e-12, err_msg="single-step update mismatch")

    return {"ok": True}

def main():
    res1 = verify_invariants_and_remaps()
    print("Invariant+remap checks:", {k: int(v) if isinstance(v, (np.integer,)) else v for k, v in res1.items()})
