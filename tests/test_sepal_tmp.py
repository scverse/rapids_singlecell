...
def synthetic_hex_data():
    ...
    spatial_coords = np.array([
        [0, 0],      # center
        [1, 0],      # right
        [0.5, 0.866], [0.5, -0.866],  # top, bottom
        [-0.5, 0.866], [-0.5, -0.866],  # top-left, bottom-left
        [-1, 0]      # left
    ], dtype=np.float32)
...
def test_sepal_validation_errors(synthetic_spatial_data):
    ...
    with pytest.raises(ValueError, match="Expected `max_neighs` to be either `4` or `6`"):
...
def test_sepal_connectivity_mismatch(synthetic_spatial_data):
    ...
    with pytest.raises(ValueError, match="Expected `max_neighs=4`, found node with `5` neighbors"):
        rsc.gr.sepal(adata, max_neighs=4, copy=True)
