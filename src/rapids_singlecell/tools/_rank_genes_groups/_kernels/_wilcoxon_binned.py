 ...
 const int gene_start)
{
    ...
    // Pre-fill: this cell contributes to bin 0 for all batch genes
    for (int g = threadIdx.x; g < n_genes; g += blockDim.x)
        atomicAdd(&hist[(long long)g * n_groups * nbt + grp * nbt], 1u);

    // Correct nonzeros: only process columns in [gene_start, gene_stop)
    for (int i = row_start + threadIdx.x; i < row_end; i += blockDim.x) {
        ...
        if (val == 0.0) continue;  // explicit zero stays in bin 0

        const int gene = col - gene_start;
        int raw = (int)((val - bin_low) * inv_bin_width);
        int bin = min(max(raw, 0), n_bins - 1) + 1;

        atomicAdd(&hist[(long long)gene * n_groups * nbt + grp * nbt], (unsigned int)(-1));
 ...
 const int gene_start)
{
    ...
    // Pre-fill: every cell contributes to bin 0 for its group
    for (int c = threadIdx.x; c < n_cells; c += blockDim.x) {
        int grp = gcodes[c];
        atomicAdd(&dst[grp * nbt], 1u);
    }

    // Correct nonzeros: move from bin 0 to the actual bin
    for (int i = col_start + threadIdx.x; i < col_end; i += blockDim.x) {
        ...
        atomicAdd(&dst[grp * nbt], (unsigned int)(-1));
        atomicAdd(&dst[grp * nbt + bin], 1u);
    }
}
"""
