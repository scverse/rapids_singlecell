...
static inline void launch_nan_mean_minor(const int* index, const T* data, double* means, int* nans,
                                         const bool* mask, int nnz, cudaStream_t stream) {
  dim3 block(32);
  dim3 grid((nnz + block.x - 1) / block.x);
  nan_mean_minor_kernel<T><<<grid, block, 0, stream>>>(index, data, means, nans, mask, nnz);
}

template <typename T>
static inline void launch_nan_mean_major(const int* indptr, const int* index, const T* data,
                                         double* means, int* nans, const bool* mask, int major,
                                         int minor, cudaStream_t stream) {
  dim3 block(64);
  dim3 grid(major);
  nan_mean_major_kernel<T>
      <<<grid, block, 0, stream>>>(indptr, index, data, means, nans, mask, major, minor);
}
...
NB_MODULE(_nanmean_cuda, m) {
  ...
  m.def(
      ...
      "index"_a, "data"_a, nb::kw_only(), "means"_a, "nans"_a, "mask"_a, "nnz"_a, "stream"_a = 0);
