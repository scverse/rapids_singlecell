...
static inline void launch_qc_csc(const int* indptr, const int* index, const T* data, T* sums_cells,
                                 T* sums_genes, int* cell_ex, int* gene_ex, int n_genes,
                                 cudaStream_t stream) {
  dim3 block(32);
  dim3 grid((n_genes + block.x - 1) / block.x);
  qc_csc_kernel<T><<<grid, block, 0, stream>>>(indptr, index, data, sums_cells, sums_genes, cell_ex,
                                               gene_ex, n_genes);
}

template <typename T>
static inline void launch_qc_csr(const int* indptr, const int* index, const T* data, T* sums_cells,
                                 T* sums_genes, int* cell_ex, int* gene_ex, int n_cells,
                                 cudaStream_t stream) {
  dim3 block(32);
  dim3 grid((n_cells + block.x - 1) / block.x);
  qc_csr_kernel<T><<<grid, block, 0, stream>>>(indptr, index, data, sums_cells, sums_genes, cell_ex,
                                               gene_ex, n_cells);
}

template <typename T>
static inline void launch_qc_dense(const T* data, T* sums_cells, T* sums_genes, int* cell_ex,
                                   int* gene_ex, int n_cells, int n_genes, cudaStream_t stream) {
  dim3 block(16, 16);
  dim3 grid((n_cells + block.x - 1) / block.x, (n_genes + block.y - 1) / block.y);
  qc_dense_kernel<T><<<grid, block, 0, stream>>>(data, sums_cells, sums_genes, cell_ex, gene_ex,
                                                 n_cells, n_genes);
}

template <typename T>
static inline void launch_qc_csc_sub(const int* indptr, const int* index, const T* data,
                                     T* sums_cells, const bool* mask, int n_genes,
                                     cudaStream_t stream) {
  dim3 block(32);
  dim3 grid((n_genes + block.x - 1) / block.x);
  qc_csc_sub_kernel<T><<<grid, block, 0, stream>>>(indptr, index, data, sums_cells, mask, n_genes);
}

template <typename T>
static inline void launch_qc_csr_sub(const int* indptr, const int* index, const T* data,
                                     T* sums_cells, const bool* mask, int n_cells,
                                     cudaStream_t stream) {
  dim3 block(32);
  dim3 grid((n_cells + block.x - 1) / block.x);
  qc_csr_sub_kernel<T><<<grid, block, 0, stream>>>(indptr, index, data, sums_cells, mask, n_cells);
}

template <typename T>
static inline void launch_qc_dense_sub(const T* data, T* sums_cells, const bool* mask, int n_cells,
                                       int n_genes, cudaStream_t stream) {
  dim3 block(16, 16);
  dim3 grid((n_cells + block.x - 1) / block.x, (n_genes + block.y - 1) / block.y);
  qc_dense_sub_kernel<T><<<grid, block, 0, stream>>>(data, sums_cells, mask, n_cells, n_genes);
}
...
NB_MODULE(_qc_cuda, m) {
  ...
  m.def(
      ...
      "indptr"_a, "index"_a, "data"_a, nb::kw_only(), "sums_cells"_a, "sums_genes"_a, "cell_ex"_a,
      "gene_ex"_a, "n_genes"_a, "stream"_a = 0);
  ...
  m.def(
      ...
      "indptr"_a, "index"_a, "data"_a, nb::kw_only(), "sums_cells"_a, "sums_genes"_a, "cell_ex"_a,
      "gene_ex"_a, "n_cells"_a, "stream"_a = 0);
  ...
  m.def(
      ...
      "data"_a, nb::kw_only(), "sums_cells"_a, "sums_genes"_a, "cell_ex"_a, "gene_ex"_a,
      "n_cells"_a, "n_genes"_a, "stream"_a = 0);
  ...
  m.def(
      ...
      "indptr"_a, "index"_a, "data"_a, nb::kw_only(), "sums_cells"_a, "mask"_a, "n_genes"_a,
      "stream"_a = 0);
  ...
  m.def(
      ...
      "indptr"_a, "index"_a, "data"_a, nb::kw_only(), "sums_cells"_a, "mask"_a, "n_cells"_a,
      "stream"_a = 0);
  ...
  m.def(
      ...
      "data"_a, nb::kw_only(), "sums_cells"_a, "mask"_a, "n_cells"_a, "n_genes"_a, "stream"_a = 0);
