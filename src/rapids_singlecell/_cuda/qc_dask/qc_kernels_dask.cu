...
static inline void launch_qc_csr_cells(std::uintptr_t indptr, std::uintptr_t index,
                                       ...
                                       std::uintptr_t cell_ex, int n_cells) {
  dim3 block(32);
...
static inline void launch_qc_dense_cells(std::uintptr_t data, std::uintptr_t sums_cells,
  ...
  qc_dense_cells_kernel<T><<<grid, block>>>(reinterpret_cast<const T*>(data),
...
NB_MODULE(_qc_dask_cuda, m) {
  ...
  m.def("sparse_qc_csr_genes",
        [](std::uintptr_t index, std::uintptr_t data, std::uintptr_t sums_genes,
           std::uintptr_t gene_ex, int nnz, int itemsize) {
          if (itemsize == 4)
            launch_qc_csr_genes<float>(index, data, sums_genes, gene_ex, nnz);
          else if (itemsize == 8)
            launch_qc_csr_genes<double>(index, data, sums_genes, gene_ex, nnz);
          else
            throw nb::value_error("Unsupported itemsize");
        });
  m.def("sparse_qc_dense_cells",
