...
static inline void launch_csc_scale_diff(const int* indptr, T* data, const T* std, int ncols,
  ...
  dim3 block(64);
  dim3 grid(ncols);
  csc_scale_diff_kernel<T><<<grid, block, 0, stream>>>(indptr, data, std, ncols);
}

template <typename T>
static inline void launch_csr_scale_diff(const int* indptr, const int* indices, T* data,
                                         const T* std, const int* mask, T clipper, int nrows,
                                         cudaStream_t stream) {
  dim3 block(64);
  dim3 grid(nrows);
  csr_scale_diff_kernel<T>
      <<<grid, block, 0, stream>>>(indptr, indices, data, std, mask, clipper, nrows);
}

template <typename T>
static inline void launch_dense_scale_center_diff(T* data, const T* mean, const T* std,
                                                  const int* mask, T clipper, long long nrows,
                                                  long long ncols, cudaStream_t stream) {
  dim3 block(32, 32);
  dim3 grid((unsigned)((nrows + block.x - 1) / block.x),
            (unsigned)((ncols + block.y - 1) / block.y));
  dense_scale_center_diff_kernel<T>
      <<<grid, block, 0, stream>>>(data, mean, std, mask, clipper, nrows, ncols);
}

template <typename T>
static inline void launch_dense_scale_diff(T* data, const T* std, const int* mask, T clipper,
                                           long long nrows, long long ncols, cudaStream_t stream) {
  dim3 block(32, 32);
  dim3 grid((unsigned)((nrows + block.x - 1) / block.x),
            (unsigned)((ncols + block.y - 1) / block.y));
  dense_scale_diff_kernel<T><<<grid, block, 0, stream>>>(data, std, mask, clipper, nrows, ncols);
}
...
NB_MODULE(_scale_cuda, m) {
  ...
  m.def(
      ...
      "indptr"_a, "data"_a, "std"_a, nb::kw_only(), "ncols"_a, "stream"_a = 0);
  ...
  m.def(
      ...
      "data"_a, "std"_a, "mask"_a, nb::kw_only(), "clipper"_a, "nrows"_a, "ncols"_a,
      "stream"_a = 0);
