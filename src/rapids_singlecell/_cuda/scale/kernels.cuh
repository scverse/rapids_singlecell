...
__global__ void csr_scale_diff_kernel(const int* __restrict__ indptr,
                                      ...
                                      const T* __restrict__ std, const int* __restrict__ mask,
                                      T clipper, int nrows) {
  int row = blockIdx.x;
  if (row >= nrows) return;
  if (mask[row]) {
...
template <typename T>
__global__ void dense_scale_center_diff_kernel(T* data, const T* __restrict__ mean,
...
template <typename T>
__global__ void dense_scale_diff_kernel(T* __restrict__ data, const T* __restrict__ std,
                                        const int* __restrict__ mask, T clipper, long long nrows,
                                        long long ncols) {
  long long row = (long long)(blockIdx.x * blockDim.x + threadIdx.x);
  long long col = (long long)(blockIdx.y * blockDim.y + threadIdx.y);
  if (row < nrows && col < ncols) {
