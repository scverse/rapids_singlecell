...
struct Tier1Config {
    ...
    bool any_tier2 = false;  // any group needs Tier 2: (T0, T2]
    bool any_above_t2 =
...
static Tier1Config make_tier1_config(const int* h_grp_offsets, int n_groups) {
    ...
    for (int g = 0; g < n_groups; g++) {
        ...
        if (sz > TIER0_GROUP_THRESHOLD && sz <= TIER2_GROUP_THRESHOLD) {
            c.any_tier2 = true;
        }
...
static inline void launch_tier0(const float* ref_sorted, const float* grp_dense,
    ...
    ovo_warp_sort_rank_kernel<<<grid, 256, 0, stream>>>(
        ref_sorted, grp_dense, grp_offsets, rank_sums, tie_corr, n_ref,
        n_all_grp, sb_cols, K, compute_tie_corr);
