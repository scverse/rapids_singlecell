 ...
 * Formula: tc = 1 - sum(t^3 - t) / (n^3 - n) where t is the count of tied values.
 *
...
__global__ void tie_correction_kernel(const double* __restrict__ sorted_vals,
                                      double* __restrict__ correction, const int n_rows,
                                      const int n_cols) {
 ...
 * Uses scipy.stats.rankdata 'average' method: ties get the average of the ranks they would span.
 *
...
__global__ void average_rank_kernel(const double* __restrict__ sorted_vals,
                                    const int* __restrict__ sorter, double* __restrict__ ranks,
