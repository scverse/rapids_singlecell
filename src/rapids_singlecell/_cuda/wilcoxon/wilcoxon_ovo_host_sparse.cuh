...
static void ovo_streaming_csr_host_impl(
    ...
    for (int i = 0; i < n_ref; i++) {
        ...
        int nnz_i = (int)(h_indptr[r + 1] - h_indptr[r]);
    ...
    for (const Pack& pk : packs) {
        ...
        int pack_items = pk.n_rows * pk.sb_cols;
    ...
    float* d_ref_sorted = pool.alloc<float>((size_t)n_ref * n_cols);
    {
        rmm::device_buffer ref_data_f32_buf(ref_nnz * sizeof(float),
                                            rmm::cuda_stream_default, pool.mr);
        rmm::device_buffer ref_indices_buf(ref_nnz * sizeof(int),
                                           rmm::cuda_stream_default, pool.mr);
        rmm::device_buffer ref_indptr_buf((n_ref + 1) * sizeof(int),
                                          rmm::cuda_stream_default, pool.mr);
        rmm::device_buffer ref_dense_buf((size_t)n_ref * n_cols * sizeof(float),
                                         rmm::cuda_stream_default, pool.mr);
        rmm::device_buffer ref_seg_buf((n_cols + 1) * sizeof(int),
                                       rmm::cuda_stream_default, pool.mr);
        ...
        // Extract ref dense (F-order) — identity row-ids
        cudaMemsetAsync(d_ref_dense, 0, (size_t)n_ref * n_cols * sizeof(float));
        {
            rmm::device_buffer ref_row_ids_buf(
                n_ref * sizeof(int), rmm::cuda_stream_default, pool.mr);
            int* d_ref_identity = (int*)ref_row_ids_buf.data();
            fill_linear_offsets_kernel<<<(n_ref + UTIL_BLOCK_SIZE - 1) /
                                             UTIL_BLOCK_SIZE,
                                         UTIL_BLOCK_SIZE>>>(d_ref_identity,
                                                            n_ref - 1, 1);
            CUDA_CHECK_LAST_ERROR(fill_linear_offsets_kernel);
            int tpb = round_up_to_warp(n_ref);
            int blk = (n_ref + tpb - 1) / tpb;
            csr_extract_dense_kernel<float>
                <<<blk, tpb>>>(d_ref_data_f32, d_ref_indices, d_ref_indptr,
                               d_ref_identity, d_ref_dense, n_ref, 0, n_cols);
        ...
        size_t ref_cub_bytes = 0;
        {
            ...
            auto* doff = reinterpret_cast<int*>(1);
            cub::DeviceSegmentedRadixSort::SortKeys(
                nullptr, ref_cub_bytes, fk, fk, (int)((size_t)n_ref * n_cols),
                n_cols, doff, doff + 1, BEGIN_BIT, END_BIT);
        }
        rmm::device_buffer cub_temp_buf(ref_cub_bytes, rmm::cuda_stream_default,
                                        pool.mr);
        upload_linear_offsets(d_ref_seg, n_cols, n_ref, 0);
        size_t temp = ref_cub_bytes;
        cub::DeviceSegmentedRadixSort::SortKeys(
            cub_temp_buf.data(), temp, d_ref_dense, d_ref_sorted,
            (int)((size_t)n_ref * n_cols), n_cols, d_ref_seg, d_ref_seg + 1,
            BEGIN_BIT, END_BIT);
        cudaDeviceSynchronize();
    }  // ref scratch drops here

    // Identity row-ids [0..max_group_rows-1] — shared across all packs.
    int* d_identity_row_ids = pool.alloc<int>(max_group_rows);
    fill_linear_offsets_kernel<<<(max_group_rows + UTIL_BLOCK_SIZE - 1) /
                                     UTIL_BLOCK_SIZE,
                                 UTIL_BLOCK_SIZE>>>(d_identity_row_ids,
                                                    max_group_rows - 1, 1);
    CUDA_CHECK_LAST_ERROR(fill_linear_offsets_kernel);
    ...
    if (may_need_cub && max_sub_items > 0) {
        auto* fk = reinterpret_cast<float*>(1);
        auto* doff = reinterpret_cast<int*>(1);
        int max_segments = max_pack_K * max_pack_sb_cols;
        cub::DeviceSegmentedRadixSort::SortKeys(
            nullptr, cub_grp_bytes, fk, fk, (int)max_sub_items, max_segments,
            doff, doff + 1, BEGIN_BIT, END_BIT);
    }
    ...
    std::vector<StreamBuf> bufs(n_streams);
    int max_pack_kernel_seg = max_pack_K * max_pack_sb_cols;
    for (int s = 0; s < n_streams; s++) {
    ...
    cudaDeviceSynchronize();  // ensure Phase 1 done before Phase 2 streams
    ...
    for (int p = 0; p < (int)packs.size(); p++) {
        ...
        while (col < n_cols) {
            ...
            int sb_items = pack_rows * sb_cols;

            cudaMemsetAsync(buf.d_grp_dense, 0, sb_items * sizeof(float),
                            stream);
            {
                int tpb = round_up_to_warp(pack_rows);
                int blk = (pack_rows + tpb - 1) / tpb;
                csr_extract_dense_kernel<float><<<blk, tpb, 0, stream>>>(
                    buf.d_grp_data_f32, buf.d_grp_indices, buf.d_grp_indptr,
                    d_identity_row_ids, buf.d_grp_dense, pack_rows, col,
                    col + sb_cols);
                CUDA_CHECK_LAST_ERROR(csr_extract_dense_kernel);
            }
            ...
            int skip_le = 0;
            if (pack_t1.use_tier0) {
                launch_tier0(ref_sub, buf.d_grp_dense, buf.d_pack_grp_offsets,
                             buf.d_rank_sums, buf.d_tie_corr, n_ref, pack_rows,
                             sb_cols, K, compute_tie_corr, stream);
                if (pack_t1.any_above_t0) skip_le = TIER0_GROUP_THRESHOLD;
            }
            if (pack_t1.any_above_t0) {
                if (compute_tie_corr) {
                    launch_ref_tie_sums(ref_sub, buf.d_ref_tie_sums, n_ref,
                                        sb_cols, stream);
                }
                launch_tier2_medium(
            ...
            if (pack_has_above_t2 && pack_t1.use_tier1) {
                ...
                int n_seg = pack_n_sort_groups * sb_cols;
                {
