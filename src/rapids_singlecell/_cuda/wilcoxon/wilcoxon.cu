...
static inline void launch_tie_correction(const double* sorted_vals, double* correction, int n_rows,
                                         int n_cols, cudaStream_t stream) {
  int threads_per_block = round_up_to_warp(n_rows);
  dim3 block(threads_per_block);
  dim3 grid(n_cols);
  tie_correction_kernel<<<grid, block, 0, stream>>>(sorted_vals, correction, n_rows, n_cols);
}

static inline void launch_average_rank(const double* sorted_vals, const int* sorter, double* ranks,
                                       int n_rows, int n_cols, cudaStream_t stream) {
  int threads_per_block = round_up_to_warp(n_rows);
  dim3 block(threads_per_block);
  dim3 grid(n_cols);
  average_rank_kernel<<<grid, block, 0, stream>>>(sorted_vals, sorter, ranks, n_rows, n_cols);
}
...
NB_MODULE(_wilcoxon_cuda, m) {
  ...
  m.def(
      ...
      "sorted_vals"_a, "correction"_a, nb::kw_only(), "n_rows"_a, "n_cols"_a, "stream"_a = 0);
  ...
  m.def(
      ...
      "sorted_vals"_a, "sorter"_a, "ranks"_a, nb::kw_only(), "n_rows"_a, "n_cols"_a,
      "stream"_a = 0);
}
