...
static size_t compute_shared_mem(int cell_tile, int l_pad, int warps_per_block) {
  size_t b_tile_bytes = static_cast<size_t>(cell_tile) * 2 * FLOAT32_SIZE;
  size_t warp_hist_bytes = static_cast<size_t>(warps_per_block) * l_pad * INT32_SIZE;
...
// Returns (cell_tile, l_pad, block_size, shared_mem_bytes) or None if insufficient memory
// Strategy: Find the best block_size first, then find the largest cell_tile that fits
static nb::object get_kernel_config(int l_val, int n_cells, int k) {
  ...
  // Try target block size first, then fall back to other sizes in preference order
  // Preference order: target, then other sizes from BLOCK_SIZES array
  ...
  for (int bi = 0; bi < num_to_try; ++bi) {
    ...
    for (int ti = 0; ti < NUM_CELL_TILE_SIZES; ++ti) {
      ...
      size_t total_shared = compute_shared_mem(cell_tile, l_pad, warps_per_block);

      if (total_shared <= max_shared) {
        // Found a valid configuration
        return nb::make_tuple(cell_tile, l_pad, block_size, static_cast<int>(total_shared));
      }
    }
...
static void launch_csr_catpairs_kernel(const float* spatial, const float* thresholds,
                                       const int* cat_offsets, const int* cell_indices,
                                       const int* pair_left, const int* pair_right, int* counts,
                                       int num_pairs, int k, int l_val, int blocks_per_pair,
                                       int l_pad, int block_size, size_t shared_mem,
                                       cudaStream_t stream) {
  dim3 grid(num_pairs, blocks_per_pair);
  dim3 block(block_size);
  occur_count_kernel_csr_catpairs_tiled<CELL_TILE><<<grid, block, shared_mem, stream>>>(
      spatial, thresholds, cat_offsets, cell_indices, pair_left, pair_right, counts, k, l_val,
      blocks_per_pair, l_pad);
}

// Dispatch to correct template specialization based on cell_tile
// l_pad is passed to the kernel as a runtime parameter
static void dispatch_csr_catpairs(const float* spatial, const float* thresholds,
                                  const int* cat_offsets, const int* cell_indices,
                                  const int* pair_left, const int* pair_right, int* counts,
                                  int num_pairs, int k, int l_val, int blocks_per_pair,
                                  int cell_tile, int block_size, size_t shared_mem,
                                  cudaStream_t stream) {
  ...
  switch (cell_tile) {
    case 1024:
      launch_csr_catpairs_kernel<1024>(spatial, thresholds, cat_offsets, cell_indices, pair_left,
                                       pair_right, counts, num_pairs, k, l_val, blocks_per_pair,
                                       l_pad, block_size, shared_mem, stream);
      break;
    case 512:
      launch_csr_catpairs_kernel<512>(spatial, thresholds, cat_offsets, cell_indices, pair_left,
                                      pair_right, counts, num_pairs, k, l_val, blocks_per_pair,
                                      l_pad, block_size, shared_mem, stream);
      break;
    case 256:
      launch_csr_catpairs_kernel<256>(spatial, thresholds, cat_offsets, cell_indices, pair_left,
                                      pair_right, counts, num_pairs, k, l_val, blocks_per_pair,
                                      l_pad, block_size, shared_mem, stream);
      break;
    case 128:
      launch_csr_catpairs_kernel<128>(spatial, thresholds, cat_offsets, cell_indices, pair_left,
                                      pair_right, counts, num_pairs, k, l_val, blocks_per_pair,
                                      l_pad, block_size, shared_mem, stream);
      break;
    case 64:
      launch_csr_catpairs_kernel<64>(spatial, thresholds, cat_offsets, cell_indices, pair_left,
                                     pair_right, counts, num_pairs, k, l_val, blocks_per_pair,
                                     l_pad, block_size, shared_mem, stream);
      break;
    case 32:
      launch_csr_catpairs_kernel<32>(spatial, thresholds, cat_offsets, cell_indices, pair_left,
                                     pair_right, counts, num_pairs, k, l_val, blocks_per_pair,
                                     l_pad, block_size, shared_mem, stream);
      break;
    case 16:
    default:
      launch_csr_catpairs_kernel<16>(spatial, thresholds, cat_offsets, cell_indices, pair_left,
                                     pair_right, counts, num_pairs, k, l_val, blocks_per_pair,
                                     l_pad, block_size, shared_mem, stream);
      break;
  }
...
NB_MODULE(_cooc_cuda, m) {
  m.def("get_kernel_config", &get_kernel_config, "l_val"_a, "n_cells"_a, "k"_a,
        "Get kernel configuration (cell_tile, l_pad, block_size, shared_mem) for given "
  ...
  m.def(
      ...
      "pair_right"_a, "counts"_a, "num_pairs"_a, "k"_a, "l_val"_a, "blocks_per_pair"_a,
      "cell_tile"_a, "block_size"_a, "shared_mem"_a, "stream"_a = 0);
