...
template <typename T>
static inline void launch_sparse2dense(
    std::uintptr_t indptr_ptr,
    std::uintptr_t index_ptr,
...
{
    dim3 block(32, 32);
    dim3 grid((unsigned)((major + block.x - 1) / block.x), 32);
...
NB_MODULE(_sparse2dense_cuda, m) {
    m.def("sparse2dense", [](std::uintptr_t indptr,
                              std::uintptr_t index,
                              std::uintptr_t data,
                              std::uintptr_t out,
                              long long major,
                              ...
                              int itemsize) {
        if (itemsize == 4) {
            launch_sparse2dense<float>(indptr, index, data, out, major, minor, c_switch);
        } else if (itemsize == 8) {
            launch_sparse2dense<double>(indptr, index, data, out, major, minor, c_switch);
        } else {
