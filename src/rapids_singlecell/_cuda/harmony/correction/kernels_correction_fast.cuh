...
//   inv[i,j] = P_row0[i-1]*c_inv*P_row0[j-1] + factor[i-1]*delta(i,j)  (i,j > 0)
//
...
__global__ void compute_inv_mats_kernel(const T* __restrict__ O, T ridge_lambda,
                                        T* __restrict__ inv_mats, T* __restrict__ g_factor,
                                        T* __restrict__ g_P_row0, int n_batches, int n_clusters,
                                        int cluster_k = -1) {
  ...
  T* my_factor = (cluster_k >= 0) ? g_factor : g_factor + (size_t)k * n_batches;
  T* my_P_row0 = (cluster_k >= 0) ? g_P_row0 : g_P_row0 + (size_t)k * n_batches;
  ...
  for (int b = threadIdx.x; b < n_batches; b += blockDim.x) {
    T o_val = O[b * n_clusters + k];  // O is (n_batches, n_clusters) row-major
...
#pragma unroll
  ...
  if (threadIdx.x < 32) {
    ...
    T Nk = (static_cast<int>(threadIdx.x) < nwarps) ? s_Nk[threadIdx.x] : T(0);
    T c_neg = (static_cast<int>(threadIdx.x) < nwarps) ? s_c_neg[threadIdx.x] : T(0);
#pragma unroll
    for (int offset = 16; offset > 0; offset >>= 1) {
...
template <typename T>
__global__ void gather_column_kernel(const T* __restrict__ src, T* __restrict__ dst, int col,
