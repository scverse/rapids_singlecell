...
// ---------- batched_correction ----------
// Each thread handles one (cell, pc) pair, accumulating corrections from all clusters.
// W_all layout: (n_clusters, n_batches+1, n_pcs) row-major
template <typename T>
__global__ void batched_correction_kernel(T* __restrict__ Z, const T* __restrict__ W_all,
                                          const int* __restrict__ cats, const T* __restrict__ R,
  ...
  for (int k = 0; k < n_clusters; k++) {
    T w_val = W_all[(long long)k * n_batches_p1 * n_pcs + (cat + 1) * n_pcs + pc];
