...
static size_t get_cub_sort_temp_bytes(int n_cells) {
  ...
  cub::DeviceRadixSort::SortPairs(nullptr, bytes, du, du, di, di, n_cells, 0, 32);
  return bytes;
}

// ---------- Column-sum dispatch ----------

enum ColsumAlgo : int { COLSUM_COLUMNS = 0, COLSUM_ATOMICS = 1, COLSUM_GEMM = 2 };

template <typename T>
static inline void colsum_dispatch(int algo, cublasHandle_t handle, const T* A, T* out,
                                   const T* ones_vec, int rows, int cols, int n_sm,
                                   cudaStream_t stream) {
  T one = T(1), zero = T(0);
  ...
  switch (algo) {
    case COLSUM_COLUMNS: {
      ...
      colsum_kernel<T><<<std::min(cols, n_sm * 8), threads, 0, stream>>>(A, out, rows, cols);
      break;
    }
    case COLSUM_ATOMICS: {
      ...
      int rows_per_tile = std::max(32, (rows + target_row_tiles - 1) / target_row_tiles);
      int row_tiles = (rows + rows_per_tile - 1) / rows_per_tile;
      colsum_atomic_kernel<T><<<dim3(col_tiles, row_tiles), dim3(32, 32), 0, stream>>>(
          A, out, rows, cols, rows_per_tile);
      break;
    }
    default:  // COLSUM_GEMM
      cublas_gemv<T>(handle, CUBLAS_OP_N, cols, rows, &one, A, cols, ones_vec, 1, &zero, out, 1);
      break;
  }
}
...
template <typename T>
static inline void scatter_add_to_O(const T* R_buf, const int* cats_in, int current_bs,
                                    int n_clusters, int n_batches, int switcher, T* O,
                                    bool use_shared, size_t shared_bytes, int n_sm,
                                    cudaStream_t stream) {
  if (use_shared) {
    ...
    scatter_add_kernel<T><<<(int)((N + 255) / 256), 256, 0, stream>>>(
        R_buf, cats_in, (size_t)current_bs, (size_t)n_clusters, (size_t)switcher, O);
  }
}
...
static void clustering_loop_impl(const ClusteringArgs<T>& a) {
  ...
  size_t scatter_shared_bytes = (size_t)a.n_batches * a.n_clusters * sizeof(T);
  bool use_scatter_shared = (scatter_shared_bytes <= (size_t)prop.sharedMemPerBlock);
  ...
  for (int iter = 0; iter < a.max_iter; iter++) {
    ...
    cublas_gemm<T>(handle, CUBLAS_OP_N, CUBLAS_OP_T, a.n_pcs, a.n_clusters, a.n_cells, &one,
                   a.Z_norm, a.n_pcs, a.R, a.n_clusters, &zero, a.Y, a.n_pcs);

    l2_row_normalize_kernel<T><<<a.n_clusters, warp_aligned_bdim(a.n_pcs), 0, a.stream>>>(
        a.Y, a.Y_norm, a.n_clusters, a.n_pcs);

    // ---- Similarities: Z_norm @ Y_norm^T ----
    cublas_gemm<T>(handle, CUBLAS_OP_T, CUBLAS_OP_N, a.n_clusters, a.n_cells, a.n_pcs, &one,
                   a.Y_norm, a.n_pcs, a.Z_norm, a.n_pcs, &zero, a.similarities, a.n_clusters);

    // ---- Shuffle: random permutation via PCG hash + radix sort ----
    pcg_hash_kernel<<<grid_1d(a.n_cells, n_sm), 256, 0, a.stream>>>(a.sort_keys, a.n_cells,
                                                                    a.seed + iter);
    iota_kernel<<<grid_1d(a.n_cells, n_sm), 256, 0, a.stream>>>(a.idx_list, a.n_cells);

    cub::DeviceRadixSort::SortPairs(a.cub_temp, cub_temp_bytes, a.sort_keys, a.sort_keys_alt,
    ...
    for (int pos = 0; pos < a.n_cells; pos += a.block_size) {
      ...
      gather_rows_kernel<T><<<grid_1d(bs * a.n_clusters, n_sm), 256, 0, a.stream>>>(
          a.R, block_idx, a.R_out_buffer, bs, a.n_clusters);
      gather_int_kernel<<<grid_1d(bs, n_sm), 256, 0, a.stream>>>(a.cats, block_idx, a.cats_in, bs);

      // Remove old contribution: O -= scatter(R_in), E -= Pr_b @ R_in_sum
      colsum_dispatch<T>(a.colsum_algo, handle, a.R_out_buffer, a.R_in_sum, a.ones_vec, bs,
                         a.n_clusters, n_sm, a.stream);
      scatter_add_to_O<T>(a.R_out_buffer, a.cats_in, bs, a.n_clusters, a.n_batches, 0, a.O,
                          use_scatter_shared, scatter_shared_bytes, n_sm, a.stream);
      outer_kernel<T><<<(ob_total + 255) / 256, 256, 0, a.stream>>>(
          a.E, a.Pr_b, a.R_in_sum, (long long)a.n_batches, (long long)a.n_clusters, 0LL);
      ...
      scatter_rows_kernel<T><<<grid_1d(bs * a.n_clusters, n_sm), 256, 0, a.stream>>>(
          a.R_out_buffer, block_idx, a.R, bs, a.n_clusters);
      colsum_dispatch<T>(a.colsum_algo, handle, a.R_out_buffer, a.R_out_sum, a.ones_vec, bs,
                         a.n_clusters, n_sm, a.stream);
      scatter_add_to_O<T>(a.R_out_buffer, a.cats_in, bs, a.n_clusters, a.n_batches, 1, a.O,
                          use_scatter_shared, scatter_shared_bytes, n_sm, a.stream);
      outer_kernel<T><<<(ob_total + 255) / 256, 256, 0, a.stream>>>(
          a.E, a.Pr_b, a.R_out_sum, (long long)a.n_batches, (long long)a.n_clusters, 1LL);
    }
  ...
  cudaMemcpyAsync(a.last_obj, &final_obj, sizeof(T), cudaMemcpyHostToDevice, a.stream);
...
static void register_clustering_loop(nb::module_& m) {
  m.def(
      "clustering_loop",
      [](cuda_array_c<const T> Z_norm, cuda_array_c<T> R, cuda_array_c<T> E, cuda_array_c<T> O,
         cuda_array_c<const T> Pr_b, cuda_array_c<const int> cats, cuda_array_c<const T> theta,
         cuda_array_c<T> Y, cuda_array_c<T> Y_norm, cuda_array_c<T> similarities,
         cuda_array_c<int> idx_list, cuda_array_c<int> idx_list_alt,
         cuda_array_c<unsigned int> sort_keys, cuda_array_c<unsigned int> sort_keys_alt,
         cuda_array_c<uint8_t> cub_temp, cuda_array_c<T> R_out_buffer, cuda_array_c<int> cats_in,
      ...
      "Z_norm"_a, nb::kw_only(), "R"_a, "E"_a, "O"_a, "Pr_b"_a, "cats"_a, "theta"_a, "Y"_a,
      "Y_norm"_a, "similarities"_a, "idx_list"_a, "idx_list_alt"_a, "sort_keys"_a,
      "sort_keys_alt"_a, "cub_temp"_a, "R_out_buffer"_a, "cats_in"_a, "R_in_sum"_a, "R_out_sum"_a,
      "penalty"_a, "obj_scalar"_a, "ones_vec"_a, "last_obj"_a, "n_cells"_a, "n_pcs"_a,
      "n_clusters"_a, "n_batches"_a, "block_size"_a, "colsum_algo"_a, "sigma"_a, "tol"_a,
