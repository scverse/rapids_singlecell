...
template <typename T>
static inline void launch_mean_var_major(
    std::uintptr_t indptr_ptr,
    std::uintptr_t indices_ptr,
    std::uintptr_t data_ptr,
    std::uintptr_t means_ptr,
    std::uintptr_t vars_ptr,
    int major,
    int minor)
{
    dim3 block(64);
...
template <typename T>
static inline void launch_mean_var_minor(
    std::uintptr_t indices_ptr,
    std::uintptr_t data_ptr,
    std::uintptr_t means_ptr,
    std::uintptr_t vars_ptr,
...
template <typename T>
void mean_var_major_api(std::uintptr_t indptr, std::uintptr_t indices, std::uintptr_t data, std::uintptr_t means, std::uintptr_t vars, int major, int minor) {
    launch_mean_var_major<T>(indptr, indices, data, means, vars, major, minor);
}

template <typename T>
void mean_var_minor_api(std::uintptr_t indices, std::uintptr_t data, std::uintptr_t means, std::uintptr_t vars, int nnz) {
...
NB_MODULE(_mean_var_cuda, m) {
    ...
    m.def("mean_var_major", [](std::uintptr_t indptr,
    ...
    m.def("mean_var_minor", [](std::uintptr_t indices,
                                std::uintptr_t data,
                                std::uintptr_t means,
                                std::uintptr_t vars,
                                int nnz,
