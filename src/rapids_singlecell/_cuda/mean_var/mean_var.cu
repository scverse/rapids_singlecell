...
static inline void launch_mean_var_major(const int* indptr, const int* indices, const T* data,
                                         double* means, double* vars, int major, int minor,
                                         cudaStream_t stream) {
  dim3 block(64);
  dim3 grid(major);
...
template <typename T>
static inline void launch_mean_var_minor(const int* indices, const T* data, double* means,
                                         double* vars, int nnz, cudaStream_t stream) {
  int block = 256;
...
NB_MODULE(_mean_var_cuda, m) {
  ...
  m.def(
      ...
      "indptr"_a, "indices"_a, "data"_a, "means"_a, "vars"_a, nb::kw_only(), "major"_a, "minor"_a,
      "stream"_a = 0);
  ...
  m.def(
      ...
      "indices"_a, "data"_a, "means"_a, "vars"_a, nb::kw_only(), "nnz"_a, "stream"_a = 0);
