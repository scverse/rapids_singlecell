...
static inline void launch_morans_dense(const float* data_centered, const int* adj_row_ptr,
                                       const int* adj_col_ind, const float* adj_data, float* num,
...
static inline void launch_morans_sparse(const int* adj_row_ptr, const int* adj_col_ind,
                                        const float* adj_data, const int* data_row_ptr,
                                        const int* data_col_ind, const float* data_values,
                                        int n_samples, int n_features, const float* mean_array,
                                        float* num, cudaStream_t stream) {
  dim3 block(1024);
  dim3 grid(n_samples);
...
static inline void launch_gearys_dense(const float* data, const int* adj_row_ptr,
                                       const int* adj_col_ind, const float* adj_data, float* num,
                                       int n_samples, int n_features, cudaStream_t stream) {
  dim3 block(8, 8);
  dim3 grid((n_features + block.x - 1) / block.x, (n_samples + block.y - 1) / block.y);
...
static inline void launch_gearys_sparse(const int* adj_row_ptr, const int* adj_col_ind,
                                        const float* adj_data, const int* data_row_ptr,
                                        const int* data_col_ind, const float* data_values,
                                        int n_samples, int n_features, float* num,
                                        cudaStream_t stream) {
  dim3 block(1024);
  dim3 grid(n_samples);
...
static inline void launch_pre_den_sparse(const int* data_col_ind, const float* data_values, int nnz,
                                         const float* mean_array, float* den, int* counter,
                                         cudaStream_t stream) {
  dim3 block(32);
