...
NB_MODULE(_autocorr_cuda, m) {
  ...
  m.def(
      "morans_dense",
      [](cuda_array_c<const float> data_centered, cuda_array_c<const int> adj_row_ptr,
         cuda_array_c<const int> adj_col_ind, cuda_array_c<const float> adj_data,
         cuda_array_c<float> num, int n_samples, int n_features, std::uintptr_t stream) {
  ...
  m.def(
      "morans_dense",
      [](cuda_array_c<const double> data_centered, cuda_array_c<const int> adj_row_ptr,
         cuda_array_c<const int> adj_col_ind, cuda_array_c<const double> adj_data,
         cuda_array_c<double> num, int n_samples, int n_features, std::uintptr_t stream) {
        launch_morans_dense(data_centered.data(), adj_row_ptr.data(), adj_col_ind.data(),
                            adj_data.data(), num.data(), n_samples, n_features,
                            (cudaStream_t)stream);
      },
  ...
  m.def(
      "morans_sparse",
      [](cuda_array_c<const int> adj_row_ptr, cuda_array_c<const int> adj_col_ind,
         cuda_array_c<const float> adj_data, cuda_array_c<const int> data_row_ptr,
         cuda_array_c<const int> data_col_ind, cuda_array_c<const float> data_values, int n_samples,
         int n_features, cuda_array_c<const float> mean_array, cuda_array_c<float> num,
  ...
  m.def(
      "morans_sparse",
      [](cuda_array_c<const int> adj_row_ptr, cuda_array_c<const int> adj_col_ind,
         cuda_array_c<const double> adj_data, cuda_array_c<const int> data_row_ptr,
         cuda_array_c<const int> data_col_ind, cuda_array_c<const double> data_values,
         int n_samples, int n_features, cuda_array_c<const double> mean_array,
  ...
  m.def(
      ...
      [](cuda_array_c<const int> adj_row_ptr, cuda_array_c<const int> adj_col_ind,
         cuda_array_c<const float> adj_data, cuda_array_c<const int> data_row_ptr,
         cuda_array_c<const int> data_col_ind, cuda_array_c<const float> data_values, int n_samples,
         int n_features, cuda_array_c<float> num, std::uintptr_t stream) {
  ...
  m.def(
      ...
      [](cuda_array_c<const int> adj_row_ptr, cuda_array_c<const int> adj_col_ind,
         cuda_array_c<const double> adj_data, cuda_array_c<const int> data_row_ptr,
         cuda_array_c<const int> data_col_ind, cuda_array_c<const double> data_values,
         int n_samples, int n_features, cuda_array_c<double> num, std::uintptr_t stream) {
        launch_gearys_sparse(adj_row_ptr.data(), adj_col_ind.data(), adj_data.data(),
                             data_row_ptr.data(), data_col_ind.data(), data_values.data(),
                             n_samples, n_features, num.data(), (cudaStream_t)stream);
      },
  ...
  m.def(
      "pre_den_sparse",
      [](cuda_array_c<const int> data_col_ind, cuda_array_c<const float> data_values, int nnz,
         cuda_array_c<const float> mean_array, cuda_array_c<float> den, cuda_array_c<int> counter,
         std::uintptr_t stream) {
  ...
  m.def(
      "pre_den_sparse",
      [](cuda_array_c<const int> data_col_ind, cuda_array_c<const double> data_values, int nnz,
         cuda_array_c<const double> mean_array, cuda_array_c<double> den, cuda_array_c<int> counter,
         std::uintptr_t stream) {
        launch_pre_den_sparse(data_col_ind.data(), data_values.data(), nnz, mean_array.data(),
                              den.data(), counter.data(), (cudaStream_t)stream);
      },
