...
__global__ void dense_row_scale_kernel(T* __restrict__ data, int nrows, int ncols, T target_sum) {
  ...
  // Parallel row sum
  T val = (T)0;
  for (int i = threadIdx.x; i < ncols; i += blockDim.x) val += data[row_offset + i];
  ...
  // Broadcast scale
  if (threadIdx.x == 0) warp_sums[0] = (val > (T)0) ? (target_sum / val) : (T)0;
  __syncthreads();

  // Parallel multiply
  T scale = warp_sums[0];
  if (scale > (T)0) {
    for (int i = threadIdx.x; i < ncols; i += blockDim.x) data[row_offset + i] *= scale;
  }
}
...
__global__ void csr_row_scale_kernel(const int* __restrict__ indptr, T* __restrict__ data,
  ...
  T scale = warp_sums[0];
  if (scale > (T)0) {
    for (int i = start + threadIdx.x; i < end; i += blockDim.x) data[i] *= scale;
  }
}
...
__global__ void csr_sum_major_kernel(const int* __restrict__ indptr, const T* __restrict__ data,
  ...
  T val = (T)0;
  for (int i = start_idx + threadIdx.x; i < stop_idx; i += blockDim.x) val += data[i];
...
// ---------- find_hi_genes_csr ----------
// One block per row. Computes row sum, then marks genes whose value > max_fraction * row_sum.
template <typename T>
__global__ void find_hi_genes_csr_kernel(const int* __restrict__ indptr,
                                         const int* __restrict__ indices,
                                         const T* __restrict__ data, bool* __restrict__ gene_is_hi,
...
// ---------- masked_mul_csr ----------
// One block per row. Computes masked row sum (skipping masked genes), then scales all data.
template <typename T>
__global__ void masked_mul_csr_kernel(const int* __restrict__ indptr,
                                      const int* __restrict__ indices, T* __restrict__ data,
                                      const bool* __restrict__ gene_mask, int nrows, T tsum) {
  int row = blockIdx.x;
  if (row >= nrows) return;
...
__global__ void masked_sum_major_kernel(const int* __restrict__ indptr,
                                        ...
                                        const bool* __restrict__ gene_mask, T* __restrict__ sums,
                                        int major) {
  int major_idx = blockIdx.x;
  if (major_idx >= major) return;
...
template <typename T>
__global__ void prescaled_mul_csr_kernel(const int* __restrict__ indptr, T* __restrict__ data,
                                         const T* __restrict__ scales, int nrows) {
  int row = blockIdx.x;
  if (row >= nrows) return;
...
// One block per row. Multiplies each element by a pre-computed per-row scale.
template <typename T>
__global__ void prescaled_mul_dense_kernel(T* __restrict__ data, const T* __restrict__ scales,
                                           int nrows, int ncols) {
  int row = blockIdx.x;
  if (row >= nrows) return;
